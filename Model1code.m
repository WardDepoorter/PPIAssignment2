...
cacheFile_model = fullfile(HOME, 'Data', sprintf('V_model_data_nmax%d_D%d_l3_%d_rhoc%d_rhom%d.mat', nmax, D, abs(Model1.l3.bound), rho_c, rho_m));

if isfile(cacheFile_model)
    fprintf('Loading cached modeled gravity data...\n');
    S = load(cacheFile_model, 'V_model_data');
    V_model_data = S.V_model_data;
else
    ...
    % Save to cache
    save(cacheFile_model, 'V_model_data', '-v7.3');
...
thickness = topography + D*ones(size(topography));
...
figCacheFile = fullfile(HOME, 'Results', 'Model1.png');

if isfile(figCacheFile)
    fprintf('Model1 figure already exists. Skipping save.\n');
else
    fprintf('Saving Model1 figure...\n');
